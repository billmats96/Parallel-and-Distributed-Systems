...
__global__ void perform_meanshift(double *x, double *y,double *y_new, double*m,double h,int totalPoints,int numOfCoordinates,int *k);
...
//Define these two parametres according to problem specifications.
#define THREADS 1024  
#define MAXROWS 5
...
double *matlabY;
//int MAXROWS,THREADS;
int numOfCoordinates,totalPoints;

int main(int argc, char **argv)
{
	FILE *fp;
	int sz,k=0,length,threadsPerBlock,blocksPerGrid,shared_size; //host variables
	double *x, *y, *y_new,*m; //host variables
	...
	//setMAXROWS_THREADS();
	shared_size=(MAXROWS+1)*THREADS*sizeof(double); //Define shared memory to allocate. Use all 48KBytes in every case but in different ways.
...
//	}
	...
	{
		perform_meanshift<<<blocksPerGrid,threadsPerBlock>>>(d_x,d_y,d_y_new,d_m,h,totalPoints,numOfCoordinates,d_k);
	}
	...
	printf("Total time needed to execute mean shift using shared memory : %f secs\n", seq_time);
...
__global__ void perform_meanshift(double *x, double *y,double *y_new, double* m,double h,int totalPoints,int numOfCoordinates,int *k)
{
	...
	//extern __shared__ double sdata_num[];  //dynamic shared memory allocation. For numerator, we use MAXROWS*THREADS*sizeof(double) bytes
	//double *sdata_den = (double*)&sdata_num[MAXROWS*THREADS]; //For denominator we use the rest of memory which is THREADS*sizeof(double) bytes
	__shared__ double sdata_num[MAXROWS][THREADS];
	__shared__ double sdata_den[THREADS];
		...
		if(i<totalPoints)
		{
			if(j<totalPoints)
			{
				...
				for(int l=0 ; l<n ;l+=MAXROWS)
				{
					...
					__syncthreads();
					//Parallelize the sum of numerator and denominator.each thread writes in its own position in shared memory
					//Set initial content to zero
					for(int c=0; c<temp; c++)
					{
						sdata_num[c][j]=0;
					}
					if (l==0) sdata_den[j]=0;
					...
					for(int q=j ;q<totalPoints;q+=THREADS)
					{
						...
						for(int c=0; c<temp; c++)
						{
							sdata_num[c][j]+=temp_dist*x[n*q+l+c];//all threads do in parallel the multiplication. for every coordinate (l+c)
						}
						if(l==0) sdata_den[j]+=temp_dist; //check if l==0 because we want to calculate it once for each yi.
						__syncthreads();
					}
					__syncthreads();
					...
					for (unsigned int s=blockDim.x/2; s>0; s>>=1)
					{
					    if (j<s)
						{
					    	for(int c=0; c<temp; c++)
					    	{
					    	   sdata_num[c][j]+=sdata_num[c][j+s];
					    	}
					    	if(l==0) sdata_den[j]+=sdata_den[j+s];
						}
					    __syncthreads();
					}
				if(l==0) den=sdata_den[0];
				if(j==0)
				{
					for(int c=0;c<temp;c++)
					{
						y_new[n*i+l+c]=sdata_num[c][0];  //here the index of y_new has to be n*i+l+c
					}
				}
				__syncthreads();
				}
