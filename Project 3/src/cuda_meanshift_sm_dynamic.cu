...
__global__ void perform_meanshift(double *x, double *y,double *y_new, double*m,double h,int totalPoints,int numOfCoordinates,int MAXROWS,int THREADS,int *k);
...
//	}
	...
	{
		perform_meanshift<<<blocksPerGrid,threadsPerBlock,shared_size>>>(d_x,d_y,d_y_new,d_m,h,totalPoints,numOfCoordinates,MAXROWS,THREADS,d_k);
	}
	...
	for(int i=(length-numOfCoordinates*50) ; i<length; i++)
	{
			...
			if(j<totalPoints)
			{
				...
				for(int l=0 ; l<n ;l+=MAXROWS)
				{
					...
					for(int q=j ;q<totalPoints;q+=THREADS)
					{
						...
						for(int c=0; c<temp; c++)
						{
							sdata_num[j+c*THREADS]+=temp_dist*x[n*q+l+c];//all threads do in parallel the multiplication. for every coordinate (l+c)
						}
					...
					for (unsigned int s=blockDim.x/2; s>0; s>>=1)
					{
					    if (j<s)
						{
					    	for(int c=0; c<temp; c++)
					    	{
					    	   sdata_num[j+c*THREADS]+=sdata_num[j+c*THREADS+s];
					    	}
				...
				if(j==0)
				{
					for(int c=0;c<temp;c++)
					{
						y_new[n*i+l+c]=sdata_num[0+c*THREADS];  //here the index of y_new has to be n*i+l+c
					}
				}
				__syncthreads();
				}
