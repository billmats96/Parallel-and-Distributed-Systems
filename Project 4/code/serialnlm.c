/*
 * Non Local Means - Serial Implementation
 *
...
float* filter_image(float* newim, float** neighbors,int m, int n,int patch_size,int patchsize_x,int offsetx, int offsety, float filtSigma);
...
int main(int argc, char **argv){
	...
	float *gaussian,**neighbors,**im,*newim;
	...
	gettimeofday (&startwtime, NULL);
	newim=filter_image(newim,neighbors,m,n,patch_size,patchsize_x,offsetx,offsety,filtSigma);
	gettimeofday (&endwtime, NULL);

	printf("\n");
	seq_time += (double)((endwtime.tv_usec - startwtime.tv_usec)/1.0e6+ endwtime.tv_sec - startwtime.tv_sec);
	//printf("Total time needed to filter image using cpu : %f secs\n", seq_time);
	printf("Total time needed using serial nlmeans: %f secs\n", seq_time);
...
float* filter_image(float *newim, float** neighbors,int m, int n,int patch_size,int patchsize_x,int offsetx, int offsety, float filtSigma)
{
	float filtVar = filtSigma*filtSigma;
	float dist;
	int k=m*n;

	float *sumofrow=(float *)calloc(k,sizeof(float));
